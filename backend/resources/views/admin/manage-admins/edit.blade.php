                ...
                <div class="grid md:grid-cols-3 gap-[1rem] md:gap-[2rem]">
                    <div class="h-fit md:col-span-2 shadow px-[3rem] py-[1.5rem] bg-[#66666622] space-y-[1.5rem]">
                        <div class="grid lg:grid-cols-12">
                            <label for="name" class="lg:col-span-2 text-gray-900 pt-[.5rem] mb-0 lg:text-right">Name:</label>
                            <div class="lg:col-span-1"></div>
                            <div class="lg:col-span-9">
                                <input type="text" name="name" value="{{nameDeformat($data->name)}}" id="name" class="h-[39px] px-[9px] py-[12px] text-[13px] text-[#555555] border border-[#eeeeee] w-full" required autofocus>
                            </div>
                        </div>
                        <div class="grid lg:grid-cols-12">
                            ...
                            <div class="lg:col-span-1"></div>
                            <div class="lg:col-span-9">
                                <input type="email" name="email" value="{{$data->email}}" id="email" class="h-[39px] px-[9px] py-[12px] text-[13px] text-[#555555] border border-[#eeeeee] w-full" required>
                            </div>
                        </div>
                        <div class="grid lg:grid-cols-12">
                    ...
                    <div class="h-fit md:col-span-1 shadow space-y-[1.5rem]">
                            ...
                            <div class="bg-white p-[.5rem] flex flex-col gap-[.2rem]">
                                ...
                                <select name="role" id="role" required>
                                    ...
                                    @foreach($roleData as $row)
                                            <option value="{{$row->id}}" {{getAdminRole($data->id) == nameDeformat($row->name) ? 'selected' : ''}}>{{$row->display_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
